...
// --- Función Principal ---
export async function generatePdfReport(results: QueueModelResults): Promise<Uint8Array> {
  ...
  const fontBold = await pdfDoc.embedFont(StandardFonts.HelveticaBold, { subset: true });
  ...
  const draw = (
      ...
      currentPage.drawText(text, { // <-- Usar currentPage
          x: margin + xOffset,
          y: currentY,
          font: fontToUse,
          size: size,
  ...
  // --- Título ---
  y -= draw('Reporte de Análisis de Línea de Espera', 0, y, fontSizeTitle, page, fontBold, unimarPrimary); // <-- Pasar 'page'
  y -= 15;

  // --- Parámetros Usados ---
  y -= draw('Parámetros de Entrada', 0, y, fontSizeHeader, page, fontBold, unimarSecondary); // <-- Pasar 'page'
  y -= 5;
  const paramX = 10;
  const valueX = 180;
  y -= draw('Modelo:', paramX, y, fontSizeBody, page, fontBold); // <-- Pasar 'page'
  draw(`${results.modelType === 'finite' ? `M/M/1/${results.params.N}` : 'M/M/1'}`, valueX, y + fontSizeBody * 1.2, fontSizeBody, page); // <-- Pasar 'page'
  y -= fontSizeBody * 1.2 + 3;

  y -= draw('Tasa de Llegada (Lambda):', paramX, y, fontSizeBody, page, fontBold); // <-- Pasar 'page'
  draw(`${results.params.lambda} (clientes/ud. tiempo)`, valueX, y + fontSizeBody * 1.2, fontSizeBody, page); // <-- Pasar 'page'
  y -= fontSizeBody * 1.2 + 3;

  y -= draw('Tasa de Servicio (Mu):', paramX, y, fontSizeBody, page, fontBold); // <-- Pasar 'page'
  draw(`${results.params.mu} (clientes/ud. tiempo)`, valueX, y + fontSizeBody * 1.2, fontSizeBody, page); // <-- Pasar 'page'
  y -= fontSizeBody * 1.2 + 3;

  if (results.modelType === 'finite' && results.params.N) {
    y -= draw('Capacidad Sistema (N):', paramX, y, fontSizeBody, page, fontBold); // <-- Pasar 'page'
    draw(`${results.params.N}`, valueX, y + fontSizeBody * 1.2, fontSizeBody, page); // <-- Pasar 'page'
    y -= fontSizeBody * 1.2 + 3;
  }
  y -= 15;

  // --- Métricas Calculadas ---
  y -= draw('Métricas de Desempeño', 0, y, fontSizeHeader, page, fontBold, unimarSecondary); // <-- Pasar 'page'
  y -= 5;
  const metricX = 10;
  const metricValueX = 250;

  y -= draw('Factor de Utilización (Rho):', metricX, y, fontSizeBody, page, fontBold); // <-- Pasar 'page'
  draw(formatNum(results.rho), metricValueX, y + fontSizeBody * 1.2, fontSizeBody, page); // <-- Pasar 'page'
  y -= fontSizeBody * 1.2 + 3;

  y -= draw('Probabilidad Sistema Vacío (P0):', metricX, y, fontSizeBody, page, fontBold); // <-- Pasar 'page'
  draw(formatNum(results.p0, 5), metricValueX, y + fontSizeBody * 1.2, fontSizeBody, page); // <-- Pasar 'page'
  y -= fontSizeBody * 1.2 + 3;

  y -= draw('Clientes Promedio en Sistema (Ls):', metricX, y, fontSizeBody, page, fontBold); // <-- Pasar 'page'
  draw(`${formatNum(results.ls)} clientes`, metricValueX, y + fontSizeBody * 1.2, fontSizeBody, page); // <-- Pasar 'page'
  y -= fontSizeBody * 1.2 + 3;

  y -= draw('Clientes Promedio en Cola (Lq):', metricX, y, fontSizeBody, page, fontBold); // <-- Pasar 'page'
  draw(`${formatNum(results.lq)} clientes`, metricValueX, y + fontSizeBody * 1.2, fontSizeBody, page); // <-- Pasar 'page'
  y -= fontSizeBody * 1.2 + 3;

  y -= draw('Tiempo Promedio en Sistema (Ws):', metricX, y, fontSizeBody, page, fontBold); // <-- Pasar 'page'
  draw(`${formatNum(results.ws)} uds. de tiempo`, metricValueX, y + fontSizeBody * 1.2, fontSizeBody, page); // <-- Pasar 'page'
  y -= fontSizeBody * 1.2 + 3;

  y -= draw('Tiempo Promedio en Cola (Wq):', metricX, y, fontSizeBody, page, fontBold); // <-- Pasar 'page'
  draw(`${formatNum(results.wq)} uds. de tiempo`, metricValueX, y + fontSizeBody * 1.2, fontSizeBody, page); // <-- Pasar 'page'
  y -= fontSizeBody * 1.2 + 3;

  if (results.lambdaEff !== undefined) {
     y -= draw('Tasa Efectiva de Llegada (Lambda_eff):', metricX, y, fontSizeBody, page, fontBold); // <-- Pasar 'page'
     draw(`${formatNum(results.lambdaEff)} clientes/ud. tiempo`, metricValueX, y + fontSizeBody * 1.2, fontSizeBody, page); // <-- Pasar 'page'
     y -= fontSizeBody * 1.2 + 3;
  }
  y -= 15;

  // --- Tabla de Probabilidades ---
  y -= draw('Distribución de Probabilidad P(n)', 0, y, fontSizeHeader, page, fontBold, unimarSecondary); // <-- Pasar 'page'
  y -= 8;
  ...
  page.drawLine({ // <-- Dibujar línea en la 'page' actual
     start: { x: margin, y: y },
...
export function downloadPdf(bytes: Uint8Array, filename: string = 'reporte-colas.pdf') {
