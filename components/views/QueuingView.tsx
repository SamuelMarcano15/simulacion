...
import { QueueModelParams, QueueModelResults, CalculationError } from "@/lib/types";
...
const formatNum = (num?: number, decimals = 4): string => {
  ...
  if (Math.abs(num) < 1e-6 && num !== 0) return num.toExponential(decimals > 0 ? decimals - 1 : 0);
...
export const QueuingView = () => {
    ...
    setTimeout(() => {
      try {
        ...
        switch (selectedModel) {
          case "MM1": calculationResult = calculateMM1Infinite(lambda, mu); break;
          case "MM1N": calculationResult = calculateMM1Finite(lambda, mu, N!); break;
          case "MMc": calculationResult = calculateMMcInfinite(lambda, mu, c!); break;
          case "MMcN": calculationResult = calculateMMcFinite(lambda, mu, c!, N!); break;
          default: throw new Error("Modelo no reconocido");
        }
  ...
  const assistantSteps = [
    { title: "Paso 1: Elige el Modelo", targetId: "asistente-paso-1", position: "top-left" as AssistantPosition, content: <p>Elige entre M/M/1, M/M/1/N, etc.</p> },
    { title: "Paso 2: Parámetros", targetId: "asistente-paso-2", position: "top-left" as AssistantPosition, content: <p>Ingresa λ (llegadas) y μ (servicio).</p> },
    { title: "Paso 3: Resultados", targetId: "asistente-paso-3", position: "bottom-center" as AssistantPosition, content: <p>Analiza ρ, Ls, Lq, Ws, Wq.</p> },
    { title: "Paso 4: Tabla Pn", targetId: "asistente-paso-4", position: "top-left" as AssistantPosition, content: <p>Distribución de probabilidad detallada.</p> },
    { title: "Paso 5: Calculadora Pn", targetId: "asistente-paso-5", position: "top-left" as AssistantPosition, content: <p>Calcula probabilidades específicas.</p> },
    { title: "Paso 6: Reporte", targetId: "asistente-paso-6", position: "center-left" as AssistantPosition, content: <p>Descarga o imprime tu reporte.</p> },
  ];
  ...
  // Avance automático
  useEffect(() => {
    if (isAssistantActive && results && assistantStep === 1) setAssistantStep(2);
  ...
  return (
    <div className="flex-1 relative">
      ...
      <div className="flex justify-end my-4 no-print">
        <Tooltip content="Presiona el botón para ayudarte con el proceso" color="foreground" placement="left">
            <Button
      ...
      <div className="mt-4 space-y-6">
        ...
        <div id="asistente-paso-1" className="bg-transparent rounded-xl p-2 transition-colors">
          <ModelSelector selectedModel={selectedModel} setSelectedModel={handleModelChange} />
        </div>
        
        {/* CORRECCIÓN AQUÍ: Agregado ID y fondo blanco */}
        <div id="asistente-paso-2" className="bg-transparent rounded-xl p-2 transition-colors">
          <CalculatorForm selectedModel={selectedModel} onSubmit={handleCalculate} isLoading={isLoading} />
        </div>

        {isLoading && <div className="flex justify-center mt-8"><Spinner color="primary" size="lg" /></div>}

        {results && !isLoading && (
          <>
            <div id="asistente-paso-3" className="bg-white rounded-xl p-2 transition-colors">
                <ResultsDisplay results={results} />
            </div>
            <div id="asistente-paso-4" className="bg-white rounded-xl p-2 transition-colors">
                <ProbabilityTable results={results} />
            </div>
            <div id="asistente-paso-5" className="bg-white rounded-xl p-2 transition-colors">
                <ProbabilityCalculator probabilities={results.probabilities} />
            </div>
            
            <div id="asistente-paso-6" className="bg-trasparent rounded-xl p-2 transition-colors">
                <motion.div 
                    initial={{ opacity: 0 }} 
                    animate={{ opacity: 1 }} 
                ...
                >
                <Button variant="bordered" color="primary" onPress={handleGeneratePdf} isLoading={isGeneratingPdf} startContent={!isGeneratingPdf ? <Icon icon="ph:file-pdf-bold" /> : null}>Generar PDF</Button>
            ...
            <AssistantWindow
                ...
                onNext={() => {
                    ...
                    if (assistantStep === 1 && !results) {
                        addToast({ title: "Acción Requerida", description: "Calcula las métricas primero.", color: "warning" });
                        return;
                    }
