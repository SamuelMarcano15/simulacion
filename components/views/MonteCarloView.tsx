...
export const MonteCarloView = () => {
  ...
  const handleSimulate = () => {
    ...
    if (isNaN(lam) || lam <= 0) {
      addToast({
        title: "Error",
        description: "Lambda debe ser un número positivo.",
        color: "danger",
      });
      return;
    }
    if (isNaN(vars) || vars < 1) {
      addToast({
        title: "Error",
        description: "Debe haber al menos 1 variable.",
        color: "danger",
      });
      return;
    }
    if (isNaN(obs) || obs < 1) {
      addToast({
        title: "Error",
        description: "Debe haber al menos 1 observación.",
        color: "danger",
      });
      return;
    }
    ...
    if (obs > 100000) {
      addToast({
        title: "Aviso",
        description: "Máximo 100,000 observaciones permitidas por rendimiento.",
        color: "warning",
      });
      return;
    }
    ...
    setTimeout(() => {
      try {
        ...
        addToast({
          title: "Error",
          description: "Falló la simulación.",
          color: "danger",
        });
      } finally {
        setIsLoading(false);
  ...
  const assistantSteps = [
    {
      ...
      content: (
        <p>
          ...
          <br />• <strong>Distribución:</strong> Elige Poisson (eventos
          discretos) o Exponencial (tiempo entre eventos).
          <br />• <strong>Lambda (λ):</strong> La tasa media de ocurrencia.
          <br />• <strong>Variables:</strong> Cuántas series independientes
          deseas generar.
    ...
    },
    {
      ...
      content: (
        <p>
          Presiona el botón para generar los números aleatorios y simular las
          variables.
        </p>
      ),
    },
    ...
    },
    {
      ...
      content: (
        <p>
          ...
          <br />• <strong>V:</strong> El valor simulado resultante tras aplicar
          la transformada inversa o algoritmo correspondiente.
        </p>
      ),
    },
    {
      ...
      content: (
        <p>Descarga un PDF con todas las observaciones de la simulación.</p>
      ),
    },
  ];
    ...
    <div className="flex-1 pb-12 relative">
      ...
      >
        <Card className="shadow-lg border border-gray-200">
          ...
          <CardBody className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div
              id="mc-step-1"
              className="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 bg-white rounded-lg p-2 transition-colors"
            >
              <Select
                ...
                variant="bordered"
              >
                <SelectItem key="POISSON" textValue="Poisson">
                  Poisson
                </SelectItem>
                <SelectItem key="EXPONENTIAL" textValue="Exponencial">
                  Exponencial
                </SelectItem>
              </Select>
            ...
            <div
              id="mc-step-2"
              className="md:col-span-2 flex justify-end mt-4 bg-white rounded-lg p-2 transition-colors"
            >
              <Button
                color="primary"
                ...
                startContent={
                  !isLoading && (
                    <Icon icon="ph:play-circle-bold" className="text-xl" />
                  )
                }
              >
      ...
      {results && (
        <div className="mt-8">
          ...
          <div
            id="mc-step-5"
            className="bg-transparent rounded-lg p-2 transition-colors"
          >
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              ...
              onNext={() => {
                if (assistantStep === 1 && !results) {
                  addToast({
                    ...
                    description: "Ejecuta la simulación primero.",
                    color: "warning",
                  });
                  return;
                }
