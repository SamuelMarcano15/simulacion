...
export const RestaurantView = () => {
  ...
  const handleApplyConfig = () => {
    if (localConfig.tableCount < 4 || localConfig.tableCount > 20) {
      addToast({
        title: "Error",
        description: "Las mesas deben ser entre 4 y 20.",
        color: "danger",
      });
      return;
    }
    if (localConfig.arrivalLambda <= 0 || localConfig.serviceMu <= 0) {
      addToast({
        title: "Error",
        description: "Las tasas deben ser positivas.",
        color: "danger",
      });
      return;
    }
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      addToast({
        title: "Error",
        description: "Fallo al generar PDF",
        color: "danger",
      });
    } finally {
  ...
  const formatTime = (totalMinutes: number) => {
    ...
    return `Día ${days + 1} - ${currentHour.toString().padStart(2, "0")}:${mins
      .toString()
      .padStart(2, "0")}`;
  };
  ...
  const assistantSteps = [
    {
      ...
      targetId: "rest-header",
      position: "bottom-left" as AssistantPosition,
      content: (
        <p>
          Este es el módulo más avanzado. Aquí simularemos un restaurante paso a
          paso usando <strong>Eventos Discretos</strong>. Podrás ver visualmente
          cómo se comporta el sistema ante diferentes cargas de trabajo.
        </p>
      ),
    },
    {
      title: "1. Configura el Escenario",
      targetId: "rest-config-params",
      position: "right-top" as AssistantPosition,
      content: (
        <p>
          ...
          <br />• <strong>Servicio (μ):</strong> Clientes por hora que atiende 1
          mesa.
          <br />• <strong>Cola (N):</strong> Capacidad máxima de espera.
          <br />
          <em>¡No olvides dar clic en "Aplicar Cambios"!</em>
        </p>
      ),
    },
    {
      title: "2. Controla el Tiempo",
      targetId: "rest-config-speed",
      position: "top-left" as AssistantPosition,
      content: (
        <p>
          La simulación puede ser lenta o rápida.
          <br />• <strong>x1:</strong> Tiempo real (lento, para análisis
          detallado).
          <br />• <strong>x100:</strong> Avance rápido (para obtener resultados
          estadísticos pronto).
        </p>
      ),
    },
    {
      ...
      targetId: "rest-controls",
      position: "bottom-right" as AssistantPosition,
      content: (
        <p>
          Usa estos controles para <strong>Iniciar</strong>,{" "}
          <strong>Pausar</strong> o <strong>Reiniciar</strong> la simulación.
          <br />
          Observa cómo el reloj avanza en el panel superior.
        </p>
      ),
    },
    {
      ...
      content: (
        <p>
          ...
          <br />• <strong>Perdidos:</strong> Clientes rechazados por aforo
          lleno.
        </p>
      ),
    },
    {
      ...
      targetId: "rest-layout",
      position: "top-left" as AssistantPosition,
      content: (
        <p>
          Aquí ocurre la magia. Observa los estados visuales:
          <br />
          🟢 <strong>Verde:</strong> Mesa Libre.
          <br />
          🔴 <strong>Rojo:</strong> Mesa Ocupada (Comiendo).
          <br />
          🟡 <strong>Amarillo:</strong> Mesa Sucia (Limpiando).
          <br />
          También verás la cola formarse en la recepción.
        </p>
      ),
    },
    {
      title: "6. Resultados y Reporte",
      targetId: "rest-stats-panel",
      position: "right-bottom" as AssistantPosition,
      content: (
        <p>
          Al finalizar o pausar, revisa los promedios acumulados aquí.
          <br />
          Cuando estés satisfecho, genera el <strong>Reporte PDF</strong>{" "}
          profesional para entregar.
        </p>
      ),
    },
  ];
  ...
  useEffect(() => {
    ...
    if (targetId) {
      ...
      if (el) {
        ...
        // Aseguramos que el elemento tenga posición relativa para que el z-index funcione
        // y un fondo blanco por si era transparente.
        el.scrollIntoView({ behavior: "smooth", block: "center" });
      }
    }
  }, [isAssistantActive, assistantStep, state]);
  ...
  return (
    ...
    >
      {/* --- ESTILOS DINÁMICOS PARA EL ASISTENTE (FIX) --- */}
      <style>{`
        .asistente-highlight {
          position: relative !important;
          z-index: 9995 !important; /* Mayor que el overlay (9990) */
          background-color: white !important; /* Fuerza fondo blanco */
          box-shadow: 0 0 0 4px #7828c8, 0 0 0 5000px rgba(0,0,0,0.5) !important; /* Borde morado y el overlay oscuro */
          border-radius: 8px;
          transition: all 0.3s ease;
          pointer-events: auto; /* Permite interactuar con el elemento */
        }
        /* Ajuste para modo oscuro si lo tuvieras */
        :global(.dark) .asistente-highlight {
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              Proyecto Final - Eventos Discretos
            </p>
          </div>
        </div>
        ...
        <div
          id="rest-hud"
          className="flex gap-4 md:gap-8 bg-gray-50 px-4 py-2 rounded-lg border border-gray-100 transition-colors"
        >
          <MetricBox
            icon="ph:clock-bold"
            label="Tiempo"
          ...
          <MetricBox
            ...
            label="En Sistema"
            value={`${state.activeCustomers.length + state.queue.length}`}
            color="text-unimar-primary"
          />
          <MetricBox
            icon="ph:chart-pie-slice-bold"
            label="Ocupación"
            value={`${(state.stats.utilization * 100).toFixed(1)}%`}
            color="text-blue-600"
          />
          <MetricBox
            icon="ph:warning-circle-bold"
            label="Perdidos"
            value={`${state.stats.customersLost}`}
            color="text-red-500"
          />
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div
            id="rest-controls"
            className="flex items-center gap-2 rounded-lg transition-colors p-1"
          >
            {!state.isRunning || state.isPaused ? (
              <Button
                color="success"
                ...
                onPress={start}
                startContent={<Icon icon="ph:play-bold" />}
              >
              ...
              <Button
                ...
                className="text-white font-bold"
                onPress={pause}
                startContent={<Icon icon="ph:pause-bold" />}
              >
                Pausar
              </Button>
            )}
      ...
      <div className="flex flex-col lg:flex-row gap-6 flex-1 min-h-0">
        ...
        <Card className="w-full lg:w-80 flex-shrink-0 border border-gray-200 shadow-sm h-full overflow-y-auto">
          <CardBody className="p-5 gap-6">
              ...
              <div className="flex flex-col gap-4">
                ...
                <Input
                  ...
                  onValueChange={(v) =>
                    setLocalConfig((prev) => ({
                      ...prev,
                      arrivalLambda: parseFloat(v) || 0,
                    }))
                  }
                  endContent={
                    <span className="text-xs text-gray-400">cli/h</span>
                  }
                />
                ...
                <Input
                  ...
                  onValueChange={(v) =>
                    setLocalConfig((prev) => ({
                      ...prev,
                      queueLimit: v ? parseInt(v) : undefined,
                    }))
                  }
            ...
            <div
              id="rest-config-speed"
              className="transition-colors p-3 rounded-lg"
            >
              <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4 flex items-center gap-2">
                <Icon icon="ph:gauge-bold" /> Velocidad (x
            ...
            <div
              id="rest-stats-panel"
              className="flex flex-col gap-4 transition-colors p-3 rounded-lg"
            >
              <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold">
                      {state.stats.avgSystemTime.toFixed(2)} min
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Tiempo Prom. Cola:</span>
                    <span className="font-bold">
                      {state.stats.avgWaitTime.toFixed(2)} min
                    </span>
                  </div>
                </div>
              </div>
              ...
              <Button
                ...
                startContent={
                  !isGeneratingPdf ? (
                    <Icon icon="ph:file-pdf-bold" className="text-lg" />
                  ) : null
                }
              >
        ...
        <div
          id="rest-layout"
          className="flex-1 min-h-[500px] flex flex-col transition-colors rounded-xl bg-white/50 backdrop-blur-sm"
        >
      ...
      <AnimatePresence>
        {isAssistantActive && (
          <>
            {/* Overlay: Eliminé la opacidad directa aquí y la manejo via CSS arriba en 'asistente-highlight' 
               o dejamos este overlay como capa base y el highlight lo supera en z-index */}
            <motion.div
              className="fixed inset-0 z-[9990] no-print pointer-events-none"
              // Quitamos bg-black/50 de aquí porque el CSS shadow del highlight hace el efecto foco,
              // O si prefieres mantener el overlay global:
            ...
            <AssistantWindow
              ...
              position={
                assistantSteps[assistantStep]?.position || "bottom-right"
              }
            />
          </>
        )}
